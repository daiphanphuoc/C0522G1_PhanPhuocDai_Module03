...
public class CustomerRepository implements ICustomerRepository {
    ...
    public Map<Integer, Customer> findAll(String sortByName) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                customer = new Customer(name, idCard, birthday, gender, phone, email, id, customerType, address);
    ...
    @Override
    public Map<Integer, Customer> find(String nameCustomer, String addressCustomer, String customerTypeName, String sortByName) {
        Map<Integer, Customer> customers = new HashMap<>();
        try {
            Connection con = ConnectionDataBase.getConnection();
            PreparedStatement pre = con.prepareStatement(SELECT_BY_SEARCH);
            pre.setString(1, nameCustomer);
            pre.setString(2, addressCustomer);
            pre.setString(3, customerTypeName);
            ResultSet rs = pre.executeQuery();
            Customer customer;
            CustomerType customerType;
            while (rs.next()) {
    ...
    public boolean update(Customer customer) {
        ...
        try {
            ...
            pre.setDate(4, (java.sql.Date) customer.getBirthday());
